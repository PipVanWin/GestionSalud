...
{
    /// <summary>
    /// Pantalla unificada de reportes (solo para Director Municipal).
    /// Recibe un enum TipoReporte al construirse para saber qué mostrar.
    ///
    /// Controles que debes crear en el diseñador:
    ///   dgvReporte       — DataGridView (ReadOnly = true)
    ///   lblTituloReporte — Label
    ///   pnlFiltros       — Panel  (contiene los filtros dinámicos)
    ///   dtpDesde         — DateTimePicker (visible solo en reporte de fechas)
    ///   dtpHasta         — DateTimePicker (visible solo en reporte de fechas)
    ///   txtCodigoEspacio — TextBox       (visible solo en reporte de espacio/buscar)
    ///   btnGenerar       — Button ("Generar reporte")
    ///   btnExportar      — Button ("Exportar a CSV")  — opcional
    /// </summary>
